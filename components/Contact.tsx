...
const Contact: React.FC = () => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result = await analyzeProjectBrief(brief);
      const cleanJson = result.replace(/```json/g, '').replace(/```/g, '').trim();
      setAnalysis(JSON.parse(cleanJson));
    } catch (err) {
      setError("Analysis unavailable. Please submit manually.");
    } finally {
  ...
  return (
    <section id="contact" className="py-24 bg-gray-50 px-6 relative overflow-hidden">
      {/* Background Accents */}
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 relative z-10">
             ...
             <div className="space-y-2">
               ...
               <div className="relative">
                 <textarea 
                    ...
                    rows={4} 
                    className="w-full bg-gray-50 rounded-xl border-0 p-4 text-gray-900 text-base focus:ring-2 focus:ring-cobalt/20 transition-all resize-none placeholder-gray-400" 
        ...
        <div className="relative h-full min-h-[500px] flex flex-col justify-center">
          <div className="relative w-full aspect-[4/5] md:aspect-square lg:aspect-auto lg:h-[600px] bg-[#0B0F17] rounded-[2.5rem] border border-gray-800 shadow-2xl overflow-hidden flex flex-col">
